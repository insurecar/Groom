...
function t3() {
    ...
    a3.map(elem => !isNaN(elem) ? a3_res.push(elem) : false);
...
function t4() {
    ...
    a4.map(elem => !isNaN(elem ? a4_res.push(elem) : false));
...
function t5() {
    let b1 = [3, 14, 15, 92];
    b1.filter(elem => !(elem & 1) ? b1_res.push(elem) : false);
...
function t6() {
    ...
    let b6_res = b6.filter(elem => typeof (elem) === 'number');
...
function t7() {
    ...
    let b7_res = b7.filter(elem => typeof (elem) === 'string');
...
function t8() {
    ...
    b8.filter((elem, index) => {
        elem % 2 === 0 ? b8_res.push(index) : false;
    });
...
function t9() {
    ...
    b9.filter(elem => typeof (elem) === 'number' ? b9_num.push(elem) : b9_string.push(elem));
    document.querySelector('.out-9').innerHTML = b9_num;
    document.querySelector('.out-91').innerHTML = b9_string;
}
...
function t10() {
    ...
    b10.map(elem => elem.filter(elemInside => elemInside === 3 ? b10_res.push(elem) : false));
    console.log(b10_res);
...
let x = arr.reduce((acum, elem, index) => {
    acum[elem] = elem;
    return acum;
}, {});
